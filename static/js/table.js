...
function renderTable(data) {
    ...
    const statPairs = [
        ['res_oq', 'res_oq_rating'], ['res_cr', 'res_cr_rating'],
        ['res_cd', 'res_cd_rating'], ['res_dr', 'res_dr_rating'],
        ['res_fl', 'res_fl_rating'], ['res_hr', 'res_hr_rating'],
        ['res_ma', 'res_ma_rating'], ['res_pe', 'res_pe_rating'],
        ['res_sr', 'res_sr_rating'], ['res_ut', 'res_ut_rating']
    ];
    ...
    data.forEach(res => {
        ...
        row.id = `row-${safeName}`;
        row.onclick = () => highlightRow(row, res.name);

        const formattedDate = formatDate(res.date_reported);
        const weightColor = getStatColorClass(res.res_weight_rating);

        // Generate Stat Cells with Tooltips
        ...
        const planetBadges = (res.planets || []).map(p => {
            ...
            return `<span class="planet ${planetLower}" data-tooltip="${p}" 
                    onclick="handleBadgeClick(event, '${res.name}', '${planetLower}')">
                    ${p.charAt(0).toUpperCase()}</span>`;
        }).join(' ');
        ...
        if (res.name === selectedResourceName) row.classList.add('highlighted-row');
    });
}
