...
def get_taxonomy():
    """
    Serves the static taxonomy file from disk.
    Bypasses IPC and DB for maximum speed.
    """
    try:
        # Load from the local file generated by generate_taxonomy.py
        with open('static/taxonomy.json', 'r') as f:
            data = json.load(f)
            
        response = jsonify(data)
        # Cache heavily in the browser (24 hours)
        response.headers['Cache-Control'] = 'public, max-age=86400'
        return response
        
    except FileNotFoundError:
        logger.error("static/taxonomy.json not found! Run generate_taxonomy.py")
        return jsonify({"error": "Taxonomy file missing"}), 500
    except Exception as e:
        logger.error(f"Error serving taxonomy: {e}")
